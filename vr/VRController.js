...
THREE.VRController.prototype.update = function(){
	...
	//let deltaPosition = new THREE.Vector3().subVectors(this.position, this.lastPos);
	let headsetOffset = new THREE.Vector3().subVectors(this.position, vrState.hmd.position);
	...
	let deltaRotation = new THREE.Quaternion().multiplyQuaternions(this.lastQuat.inverse(), this.quaternion);
	...
	//let hyperPos = new THREE.Vector4(0,0,0,1).applyMatrix4(g_controllerBoosts[gamepad.index]).applyMatrix4(g_currentBoost);
...
OrientationArmModel.prototype.update = function(){
	...
	let elbowOffset = new THREE.Vector3().copy(OrientationArmModel.ARM_EXTENSION_OFFSET);
	...
	let offset = new THREE.Vector3().copy(OrientationArmModel.ARM_EXTENSION_OFFSET);
