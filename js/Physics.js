...
//If you want to move a point by isometry X
State.prototype.translateBy = function (isom) {
    // translate the state by the given isometry
...
State.prototype.localFlow = function (t) {
    ...
    let isom = new Isometry().translateByVector(vt);
...
State.prototype.getInverse = function (state) {
    // set the current position to the position that can bring back the passed position to the origin position
    this.boost.getInverse(state.boost);
    this.facing.getInverse(state.facing);
    this.reduceError();
    this.velocity = state.velocity.clone().negate();
    this.angular = state.angular.clone().negate();
    return this;
