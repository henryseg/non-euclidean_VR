 ...
 */
class VRControlsReset extends EventDispatcher {
        ...
        if (this._update === undefined) {
            ...
            this._update = function (delta) {
                ...
                if (this._isSelecting) {
                    const quat = new Quaternion().setFromRotationMatrix(this.controller.matrixWorld);
                    quat.x = 0;
                    quat.z = 0;
                    quat.normalize();
                    console.log(quat.toLog());
                    this.position.reset();
                    this.position.local.quaternion.copy(quat).invert();
