...
//----------------------------------------------------------------------------------------------------------------------
int MAX_MARCHING_STEPS =  620;
int MAX_REFL_STEPS=50;
int MAX_SHADOW_STEPS=50;
...
uniform vec4 localLightPosition[4];
...
vec4 reflectedColor;
    ...
    //dodecahedron (v) and icosaheron (w) vertices
    vec4 v1=normalize(vec4(1.,1.,1.,0.));
    
    vec4 v2=normalize(vec4(1.,1.,-1.,0.));
    vec4 v21=normalize(vec4(-1.,1.,1.,0.));
    vec4 v22=normalize(vec4(1.,-1.,1.,0.));
    
    vec4 v3=normalize(vec4(1./1.618,0.,1.618,0.));
    vec4 v31=normalize(vec4(1.618,1./1.618,0.,0.));
    vec4 v32=normalize(vec4(0.,1.618,1./1.618,0.));
    
    vec4 v4=normalize(vec4(0.,1.618,-1./1.618,0.));
    vec4 v41=normalize(vec4(-1./1.618,0.,1.618,0.));
    vec4 v42=normalize(vec4(1.618,-1./1.618,0.,0.));
    
    vec4 w1=normalize(vec4(0.,1.,1.618,0.))+vec4(0.,0.,0.,0.);
    vec4 w2=normalize(vec4(1.618,0.,1.,0.))+vec4(0.,0.,0.,0.);
    vec4 w3=normalize(vec4(1.,1.618,0.,0.))+vec4(0.,0.,0.,0.);
    
    vec4 w4=normalize(vec4(0.,-1.,1.618,0.))+vec4(0.,0.,0.,0.);
    vec4 w5=normalize(vec4(1.618,0.,-1.,0.))+vec4(0.,0.,0.,0.);
    vec4 w6=normalize(vec4(-1.,1.618,0.,0.))+vec4(0.,0.,0.,0.);
...
void setVariables(){
   ...
   Isometry iG5=Isometry(invGeneratorsMatrices[5],invGeneratorsReals[5]);
    ...
    localLightPos=localLightPosition[0];
    localLightPos2=localLightPosition[1];
