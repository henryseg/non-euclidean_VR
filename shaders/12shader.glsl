...
vec3 allLocalLights(vec3 surfColor,bool marchShadows, Isometry fixPosition){
   ...
   // return localLight(localLightPos, localLightColor, 2.*(2.+cosh(2.*brightness)),marchShadows, surfColor,fixPosition);
        ...
        vec3 localColor=vec3(0.);
         ...
         if(res==0.){
                  localColor+=localLight(w[i], (lightIntensities[i].xyz+vec3(1.,1.,1.))/2., 0.5,true, surfColor,fixPosition);
         }    
             
        else{
         localColor+=localLight(w[i], (lightIntensities[i].xyz+vec3(1.,1.,1.))/2., 0.5,false, surfColor,fixPosition);
        }
     }
...
vec3 allGlobalLights(vec3 surfColor,bool marchShadows,Isometry fixPosition){
    ...
    vec3 globalColor=vec3(0.);
     ...
     for (int i=0; i<4; i++){
         ...
         globalColor+=globalLight(lightPositions[i], lightIntensities[i].xyz, 6.,marchShadows, surfColor,fixPosition);
     }
