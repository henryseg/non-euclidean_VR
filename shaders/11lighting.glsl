...
//    return 5.*exp(-dist)+surfArea(dist);//the gaussian makes the light not too bright right at it - so its not just a white blob!
//}
//
...
float lightAtt(float dist, tangVector angle){
    ...
    //actual distance function
//    if(dist<0.2){
//         return 5.*exp(10.*dist*dist)+areaElement(dist,angle);
//    }
    
    //adding a constant to shift away from infinity brightness breaks it
    return 0.1+0.5*dist;
...
vec3 phongShading(tangVector toLight, tangVector toViewer, tangVector  surfNormal, float distToLight, vec3 baseColor, vec3 lightColor, float lightIntensity,tangVector atLight){
    ...
    //only need w component which is unchanged under the flow AND reversal
    //thus, we can use the wrrong toLight vector here instead of atLight 
    float att = lightIntensity /(lightAtt(distToLight,toLight));
...
vec3 fog(vec3 color, vec3 fogColor, float distToViewer){
    //float fogDensity=1.-exp(-(1.-res)*distToViewer);
    //float fogDensity=smoothstep(0., MAX_DIST/25., distToViewer);
    return exp(-distToViewer/6.)*color;
...
vec3 localLight(vec4 lightPosition, vec3 lightColor, float lightIntensity,bool marchShadows, vec3 surfColor,Isometry fixPosition){
    ...
    // alternative: we may wish to  try to turn off shadows when you are far away to save steps:
     ...
     //----------------SAME CODE FOR THE ANTIPODE LIGHT---------------
    vec4 antipodeLight=vec4(-lightPosition.xyz,lightPosition.w);
        translatedLightPosition=translate(fixPosition,antipodeLight);
    ...
    for (int i=0; i<2; i++){
        ...
        //only nearest neighbor up and down the fiber.
        translatedLightPosition=translate(composeIsometry(fixPosition,invGenerators[4+i]),lightPosition);
...
//            }  
       ...
       //same for antipide
            translatedLightPosition=translate(composeIsometry(fixPosition,invGenerators[4+i]),antipodeLight);
