...
float lightAtt(float dist, Vector angle){
    ...
    return 35.*exp(-dist*dist*5.)+AreaDensity(dist,angle);
...
vec3 phongShading(Vector toLight, Vector toViewer, Vector  surfNormal, float distToLight, vec3 baseColor, vec3 lightColor, float lightIntensity,Vector atLight){
    ...
    if(hitWhich==1){//make specularity make sense for a lightsource giving some arbitrary direction
        toLight=Vector(sampletv.pos, vec3(0.,1.,1.));
    }
 Vector fromLight=turnAround(toLight);
    ...
    float correctAtt=1./lightAtt(distToLight,atLight);
    float fakeAtt=1./pow(lightAtt(distToLight,atLight),0.25);
    float att =lightIntensity*(correctAtt+0.5*fakeAtt);
    if(hitWhich==1){att=.2;}
    //Combine the above terms to compute the final color
    vec3 phongColor=(baseColor*(diffuse + .15) + vec3(.6, .5, .5)*specular*2.);
    
    return att*phongColor;
...
//global light: translate by fix position and cell boost

vec3 globalLight(Point lightPosition, vec3 lightColor, float lightIntensity, vec3 baseColor, Isometry fixPosition){
    
     Point transLightPosition;
     Isometry totalFix=composeIsometry(fixPosition, invCellBoost);
    transLightPosition=translate(totalFix,lightPosition);
    
    return Light(transLightPosition, lightColor, lightIntensity, baseColor, fixPosition);
